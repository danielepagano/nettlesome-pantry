...
function emptyProductRow() {
  return Object.fromEntries(PRODUCT_COLUMNS.map((column) => [column, ""]));
}
function emptyCustomerRow() {
  return Object.fromEntries(CUSTOMER_COLUMNS.map((column) => [column, ""]));
}

// src/transforms/customers.ts
function transformCustomers(rows, options = {}) {
  const warnings = [];
  const output = [];
...
function rewriteProductLinks(html, options) {
  const stats = { rewritten: 0, unchanged: 0, flagged: 0 };
  const entries = [];
  const baseUrl = options.baseUrl.replace(/\/$/, "");
  const pagePrefix = options.productPage.replace(/^\/+|\/+$/g, "");
  const rewritten = html.replace(/href=(["'])(.*?)\1/gi, (match, quote, href) => {
    const result = rewriteHref(href, baseUrl, pagePrefix, options.handles);
    stats[result.kind] += 1;
    entries.push({
      action: result.kind,
      originalHref: href.trim(),
      resultHref: result.kind === "rewritten" ? result.href : void 0,
...
function rewriteHref(href, baseUrl, pagePrefix, handles) {
...
function rewritePath(path, baseUrl, pagePrefix, handles, original) {
...
// src/transforms/products.ts
var POUNDS_TO_GRAMS = 453.6;
var DEFAULT_VENDOR = "Nettlesome Pantry";
var PRODUCT_TYPE_FIELD = "Product Type [Non Editable]";
function transformProducts(rows, options = {}) {
  const warnings = [];
  const output = [];
  ...
  const handles = new Set(
    includedRows.map((row) => normalizeHandle(row["Product URL"] ?? "")).filter((handle) => handle.length > 0)
  );
  let parent = null;
...
function collectPhysicalRows(rows, warnings, onSkipNonPhysical) {
  const included = [];
  let parentType = null;
  for (const row of rows) {
...
function buildVariantRow(row, parent, warnings, includeProductFields) {
  ...
  for (let index = 1; index <= 3; index += 1) {
    variant[`Option${index} Name`] = row[`Option Name ${index}`] ?? "";
    variant[`Option${index} Value`] = row[`Option Value ${index}`] ?? "";
  }
...
function mapPublished(visible) {
  return visible.trim().toLowerCase() === "yes" ? "true" : "false";
}
function mapInventoryQty(stock, warnings) {
...
function poundsToGrams(weight, warnings) {
...
function splitImageUrls(value) {
  return value.trim().split(/\s+/).map((url) => url.trim()).filter((url) => url.length > 0);
}
...
// src/convert.ts
function convertSquarespaceCsv(text, options = {}) {
...
// src/ui/app.ts
var DEFAULT_STOREFRONT_URL = "https://my-store.myshopify.com";
function initApp(root2) {
  root2.innerHTML = `
    <main class="panel">
      <header>      
  ...
  `;
  const elements = {
    form: root2.querySelector("#convert-form"),
    fileInput: root2.querySelector("#csv-file"),
    kindSelect: root2.querySelector("#export-kind"),
    countryInput: root2.querySelector("#default-country"),
    storefrontInput: root2.querySelector("#storefront-url"),
    convertButton: root2.querySelector("#convert-button"),
    summary: root2.querySelector("#summary"),
    warnings: root2.querySelector("#warnings"),
    downloadButton: root2.querySelector("#download-link")
  };
...
async function handleConvert(elements) {
  ...
  const warningsSection = elements.warnings.closest("#warnings-section");
  warningsSection.hidden = true;
  try {
    const text = await file.text();
    const selectedKind = elements.kindSelect.value;
    const result = convertSquarespaceCsv(text, {
      kind: selectedKind === "auto" ? void 0 : selectedKind,
...
function renderSummary(container, result) {
  ...
  const detailSections = result.details.filter((section) => section.items.length > 0 || (section.linkItems?.length ?? 0) > 0).map((section) => {
    const intro = section.intro ? `<p class="detail-intro">${escapeHtml(section.intro)}</p>` : "";
    const body = section.linkItems?.length ? renderLinkItems(section.linkItems) : `<ul>${section.items.slice(0, 25).map((item) => `<li>${escapeHtml(item)}</li>`).join("")}${section.items.length > 25 ? `<li>${section.items.length - 25} more not shown.</li>` : ""}</ul>`;
    return `
  ...
  const emptyDetails = result.kind === "products" && result.details.every((section) => section.items.length === 0 && (section.linkItems?.length ?? 0) === 0) ? `<p class="detail-intro">No description links were rewritten or flagged. External links and links left unchanged are not listed here.</p>` : "";
  container.innerHTML = `
...
function renderLinkItems(linkItems) {
  ...
  const overflow = linkItems.length > 25 ? `<li class="link-item">${linkItems.length - 25} more not shown.</li>` : "";
  return `<ul class="link-list">${lines}${overflow}</ul>`;
}
function renderWarnings(container, section, warnings) {
...
function prepareDownload(link, result) {
